...
=item C<create_ch_cvterm>
 ...
 doc - XML::DOM::Document required
 name - string required
 cv_id - macro id for cv or XML::DOM cv element required
 cv - string = cvname
 definition - string optional
 dbxref_id - macro id for dbxref XML::DOM dbxref element optional
...
sub create_ch_contact {
	...
	print "WARNING -- no XML::DOM::Document specified\n" and return unless $params{doc};
	my $ldoc = $params{doc};    ## XML::DOM::Document
	my $contactid_el = $ldoc->createElement('contact_id') if $params{with_id};

	my $contact_el = $ldoc->createElement('contact');
	$contact_el->setAttribute('id',$params{macro_id}) if $params{macro_id};  
	
	# add an op="lookup" attribute unless no_lookup is specified
	unless ($params{no_lookup}) {
        	$contact_el->setAttribute('op','lookup');
    	}	
	...
	foreach my $e (keys %params) {
	        ...
	        $contact_el->appendChild(_build_element($ldoc,$e,$params{$e}));
    	}	

	if ($contactid_el) {
        	$contactid_el->appendChild($contact_el);
        	return $contactid_el;
    	}
    	return $contact_el;
}
  
sub create_ch_contactprop {
	my %params = @_;
    	$params{parentname} = 'contact';
    	my $fp_el = create_ch_prop(%params);
    	return $fp_el;
}

sub create_ch_contact_relationship {
    my %params = @_;
    print "WARNING -- no XML::DOM::Document specified\n" and return unless $params{doc};
    my $ldoc = $params{doc};    ## XML::DOM::Document

    ## contact_relationship element (will be returned)
    my $fr_el = $ldoc->createElement('contact_relationship');   
    ...
    # now deal with various contact  options
    if ($params{object_id}) { 
	...
	$contact_params{type_id} = $params{type_id} if $params{type_id};
	$params{contact_id} = create_ch_contact(%contact_params);
    } # now we have a contact element to associate as subject or object    
    ...
    $fr_id->appendChild($params{contact_id});
...
sub create_ch_phendesc {
    ...
    print "ERROR -- required parameters are missing: description, type or pub info - NO GO!\n" and return
      unless ($params{description} and ($params{type_id} or $params{type}) and ($params{pub_id} or $params{pub}));
...
sub create_ch_phenotype_cvterm {
  ...
  $ect_el->appendChild(_build_element($ldoc,'cvterm_id',$params{cvterm_id})) if $params{cvterm_id};
  $ect_el->appendChild(_build_element($ldoc,'rank',$params{rank})) if $params{rank};
...
sub create_ch_pubauthor {
    ...
    print "WARNING -- missing required rank (positive integer) or surname, NO GO!\n" 
      and return unless ($params{rank} and $params{surname});
...
sub _create_simple_element {
  ...
  foreach my $e (keys %params) {
    next if ($e eq 'doc' || $e eq 'with_id' || $e eq 'macro_id' || $e eq 'lookup');
